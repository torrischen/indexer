...
index_t *find(internal_node_t &node, const key_t &key, bool flag = false) {
    if (key) {
...
int bplus_tree::update(const key_t& key, const value_t &value) {
...
void bplus_tree::remove_from_index(off_t offset, internal_node_t &node, const key_t &key, bool remove_flag) {
